...
def setup(config):
    ...
    cfg ={
            ...
            'in_channels': 3,
            'out_channels': 3,
            'model_channels': config['enc_channels'],
            'emb_channels': config['embed_dim'],
            ...
            'use_conv_resample': config['use_conv_resample'],
            'num_groups': config['num_groups'],
        }

    diffae_encoder = SemanticEncoder(cfg=cfg)

    optimizer = torch.optim.AdamW(
        list(unet.parameters()) + list(diffae_encoder.parameters()),
        lr=config["lr"],
    ...
    diffae_encoder.requires_grad_(True)
    ...
    return (unet, diffae_encoder, vae, accelerator, optimizer, train_dataloader, val_dataloader,
            lr_scheduler, noise_scheduler, max_train_steps)

def train_epoch(vae, unet, diffae_encoder, train_dataloader, accelerator, optimizer, lr_scheduler,
                noise_scheduler, config, epoch, progress_bar):
    global global_step
    unet.train()
    diffae_encoder.train()
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        with accelerator.accumulate(unet):
            ...
            encoder_hidden_states = diffae_encoder(batch[0].to(accelerator.device)).unsqueeze(1)
            ...
            if accelerator.sync_gradients:
                ...
                accelerator.clip_grad_norm_(diffae_encoder.parameters(), 1.0)
                optimizer.step()
                global_step += 1
    ...
    if checkpoint and accelerator.is_main_process:
        ...
        encoder_model = accelerator.unwrap_model(diffae_encoder)
...
def eval_epoch(vae, unet, diffae_encoder, val_dataloader, noise_scheduler, accelerator):
    unet.eval()
    diffae_encoder.eval()
    val_loss = []
    with torch.no_grad():
...
def objective(config):
    (unet, diffae_encoder, vae, accelerator, optimizer, train_dataloader, val_dataloader,
    ...
    for epoch in range(start_epoch, config["num_train_epochs"]):
        ...
        train_loss = train_epoch(vae, unet, diffae_encoder, train_dataloader, accelerator, optimizer, lr_scheduler,
                    noise_scheduler, config, epoch, progress_bar)
        if accelerator.is_main_process:
                wandb.log({"train/loss_per_epoch": train_loss, "epoch": epoch})

        if config.get("validation", False) and (epoch % 10 == 0):
            val_loss = eval_epoch(vae, unet, diffae_encoder, val_dataloader, noise_scheduler, accelerator)
            if accelerator.is_main_process:
    ...
    torch.save(diffae_encoder, final_model_path.replace("_full_model", "_diffae_encoder"))
